"""Default-on smoke-forecast entrypoint (BUILD Arc B) — the "does it forecast?" first-apply proof.
...
``src/``, launched by a thin ``gs://`` shim). It calls :func:`scale_forecasting.main.run` with the
batch's own :class:`SparkSession` **injected**: on that path (``main.run(cfg, spark=session)`` with
...
passes identity as ``--sf-*`` job args, which :func:`main` exports into ``os.environ`` before
anything resolves ``Settings`` — keeping env-based resolution the single G1 seam (parity with the
seed / ``spark_entry``).

Public surface: ``main(argv)``. ``pyspark`` and :mod:`.main` import lazily inside :func:`main`, so
...
def main(argv: list[str] | None = None) -> None:
    ...
    The batch's :class:`SparkSession` is passed straight into :func:`scale_forecasting.main.run`, so
