"""Split a run's model list by runtime (CONTRACTS §6).
...
sum (DESIGN §3.3). This module owns the run-level split that decides which engine gets which models,
...
def split_by_runtime(cfg: RunConfig) -> tuple[list[str], list[str]]:
    ...
    Unknown model names raise ``ModelError`` (via :func:`~scale_forecasting.models.get_model`) — the
