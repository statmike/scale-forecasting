...
**`gpu_cluster_image` moved on 2026-09-12 because the driver stopped building, and the honest thing
to say is that it stopped building on a kernel we never chose.** A Dataproc *cluster* with GPUs runs
Google's `install_gpu_driver.sh`, which compiles NVIDIA's open kernel modules from source unless a
prebuilt tarball for that exact kernel is already cached. NVIDIA's source calls
`pci_resize_resource` with three arguments; Debian's 6.1.0-52 headers declare a fourth, so the
compile stops at `nv-pci.o`. Every released driver fails the same way (550.142, 550.163.01,
570.172.08, 580.82.07), and so does Debian's own packaged `nvidia-open-kernel-dkms` — which means
neither the `gpu-driver-version` nor the `gpu-driver-provider` knob reaches around it. The floating
`2.2-debian12` alias had rolled onto an image that boots that kernel, so a GPU cluster that created
cleanly in early September stopped creating in mid-September with nothing in our code changed.
The fix pins the GPU path to `2.2.85-debian12`, the previous sub-minor, which boots 6.1.0-49 and
builds. CPU clusters run no driver install and keep tracking the floating line, so rows 04 and 05
are untouched. Rows 06 and 18 are the two that declared the old value; both go `STALE`, though the
...
| 21 | `21_full_catalogue.json` | **Catalogue sweep:** every registered model — all eighteen — and every ensemble strategy in one run on Serverless CPU, covering the eleven models no shipped config had ever named and the two learned strategies nothing had ever fitted outside a unit test. Every one of the 24 model kinds returned 50 scored cells with **no NULL in any of the fifteen metrics** and both folds achieved, which is the specific failure this config exists to detect: a model whose predictions are unusable raises nothing and lands a well-formed row of nothing. Row counts are exactly complete — 33,600 forecasts and 50,400 model out-of-fold rows, no NULL `yhat` in either. `xgb`'s weights came back non-negative and summing to 1.000002 and `ridge`'s signed (holtwinters +2.05 against autoets −1.34), which is the live half of the offline claim that the two are normalised importances and coefficients respectively | CURRENT | 2026-09-23 | `smoke-21-full-catalogue-98e3cb319f96` | `ensemble_weighting=per-series-calculated+batch-fit-learned`, `backtest_scoring=holdout-reserved+embargo-aware+auto-refit`, `serverless_deps=container-image`, `native_source_pin=unpinned-all-sources`, `python=3.11`, `fleet_sizing=derived-overlay-three-way-min`, `horizon_features=computed-at-future-dates`, `run_id_inputs=authored-config-only-v3` |
| 22 | `22_backtest_sliding_overlap.json` | **Backtest semantics 1 of 4:** the `sliding` scheme, the `overlap` short-series policy, and `control_arm` on a refitting scheme, decided by `rmse`. The policy is proven by the *geometry the data came back with*, not by the config naming it: the run asked for six folds of 28 and the observed cutoffs run 2024-07-25 → 2024-12-02, which is 130 days across five gaps — **a step of 26, not 28**. That is `overlap` giving up fold independence to reach all six, and every cell carries the matching `backtest_status=reduced` plus the note naming the trade. `control_arm` filled `yhat_stale` on all 3,360 out-of-fold rows per model, and the blind arm is a genuinely different and uniformly worse forecast (holtwinters MAE 13.92 refitting against 30.45 blind); the 560 rows per model where the two agree are exactly fold 0 — 20 series × 28 horizon — where a refitting arm and a blind arm must agree because no refit has happened yet. All 60 cells `ok`, no NULL in any metric | CURRENT | 2026-09-23 | `smoke-22-backtest-sliding-overlap-eca75eb54835` | `backtest_scoring=holdout-reserved+embargo-aware+auto-refit`, `serverless_deps=container-image`, `python=3.11`, `fleet_sizing=derived-overlay-three-way-min`, `job_status=derived-from-cell-tallies`, `run_id_inputs=authored-config-only-v3` |
...
*branch* one is to ask for a fold grid the data cannot meet. All four ask for six folds of 28 at a
minimum training length of 1,300, which 1,460 observations miss by a little, and each then answers
the shortfall differently: 22 shrinks the step to 26 and buys all six folds at the cost of
independence, 23 drops the training minimum to 1,292 and keeps the step, 24 takes the default and
settles for five of six, and 25 declines to score at all. A config that merely *named* a policy its
data comfortably satisfied would exercise none of this, and nothing reading configs could tell the
difference.

`short_series="error"` is deliberately absent. It refuses the panel at pre-flight rather than
running, so its proof is a submission that never happens — which belongs in a pre-flight test, not
in a table of runs.
...
things. **Placement passed:** every cell reported `device_used='cuda'`, so the routing, the
provisioning, the device probe and the audit all did their jobs, and smokes 17–19 prove the check
can object. **Utilisation is ~0.0005 %:** peak device memory was 50–87 KB against a card of 16 GB or
more, and `device_audit` stamped `ENGAGED_IDLE` on all of them. That is the expected verdict for
`neuralprophet` at its shipped `n_lags=0`; it warns rather than failing a job, because the run is
correct — it is only paying for hardware it does not need. **No row on this page has ever shown a
GPU doing useful work**, and the accelerator A/B below is what that observation was then run to
price. The contract and the cost are written up in [quota and scale](quota_and_scale.md).
...
**The fault is armed by the operator, not by the config, and forgetting it produces a green run that
proves nothing.** This was learned the expensive way on 2026-09-13: smoke 18 was re-run without
`SF_HIDE_DEVICES` set, and because the harness verifies a run rather than a *claim*, it completed
six cells normally and printed `PASS`. Nothing was wrong with the run; it simply was not the
negative arm. A negative arm that passes the harness is the one result on this page that should be
read as a failure — the expected shape is `RESULT: FAIL` with every cell refused. The fault cannot
move into the config, because a new config field would move the `run_id` of every row that declares
one, so the discipline has to live in the runbook and in the reader's head.
...
| Config | Proves | Status | Date | run_id | Axes at proof |
|--------|--------|--------|------|--------|---------------|
| `bq_native_demo.json` | The BigQuery-native family alone — no cluster of any kind (100 series) | CURRENT | 2026-09-15 | `bq-native-demo-afdc0507e8b5` | `native_source_pin=unpinned-all-sources`, `python=3.11`, `run_id_inputs=authored-config-only-v3` |
...
| `per_family_runtimes_cpu_demo.json` | The same per-family split with the deep-learning family on **CPU** — the shape this product now recommends, and the one a novice should copy. Identical to the row above in every other respect, so the two read as a diff | NEVER_RUN | — | — | — |
| `ray_cpu_demo.json` | Ray on Vertex, CPU, alongside the natives, backtested (6) | CURRENT | 2026-09-15 | `ray-cpu-demo-1650659c3c20` | `ray_pool_shape=autoscaling`, `ray_deps=stock-image+uv-runtime-env`, `ray_slot_memory=harvest-only`, `native_source_pin=unpinned-all-sources`, `backtest_scoring=holdout-reserved+embargo-aware+auto-refit`, `python=3.11`, `fleet_sizing=derived-overlay-three-way-min`, `run_id_inputs=authored-config-only-v3`, `horizon_features=computed-at-future-dates` |
| `ray_gpu_demo.json` | Ray on Vertex, GPU T4 (`neuralprophet`), alongside the natives (6). The re-run also reproduces the utilisation finding rather than hiding it: the deep-learning job's `device_verdict` came back **`ENGAGED_IDLE`** at 56,832 bytes of peak device memory. The device is reached, and it is not used — the claim this row makes is placement, and only placement | CURRENT | 2026-09-15 | `ray-gpu-demo-fa53d37c8122` | `ray_pool_shape=autoscaling`, `ray_deps=stock-image+uv-runtime-env`, `ray_slot_memory=harvest-only`, `dl_gpu_routing=resolved-per-family`, `gpu_device_probe=trainer-root-device`, `native_source_pin=unpinned-all-sources`, `backtest_scoring=holdout-reserved+embargo-aware+auto-refit`, `python=3.11`, `fleet_sizing=derived-overlay-three-way-min`, `run_id_inputs=authored-config-only-v3`, `horizon_features=computed-at-future-dates` |
...
**The same reading applies to every GPU row here.** `ray_gpu_demo`, `per_family_runtimes_demo`,
`all_families_10k`, `all_families_10k_full` and the two GPU arms of the accelerator A/B all placed
the deep-learning family on a device, and none of them used it. Across 31,356 live NeuralProphet
fits, peak device memory was 50–78 KB — **0.00045 % of a T4** — with `cpu_seconds / fit_seconds`
between 0.93 and 0.996: a CPU workload billed as a GPU workload. Placement passed on all of them;
utilisation never has. The two A/B pairs are the runs that put a price on that, and both CPU arms
won.
...
[smoke testing](smoke_testing.md#arming-the-ray-poll-recovery-sf_ray_poll_fault). **It was carried
on a live run on 2026-09-22**, and the paragraph above no longer stands: see
[the armed poll fault](#the-armed-poll-fault-fired-and-the-evidence-for-it-was-invisible-at-the-level-the-runbook-logs-at)
below, where the recovery ran against the real proxy on smoke 07 and the run completed.
...
#### 2026-09-16, re-earning that row cost 44 % of the per-device throughput it established

`all-families-10k-a0f6797d69c1` re-ran the same config on today's code and passed everything asked of
it: header `COMPLETED`, all four family jobs `COMPLETED` at attempt 1, 70,000 `forecast_metadata`
rows every one of them `cell_status = ok` with no `error_class` on any model, 1,960,000 prediction
cells (10,000 × 7 × 28), a seven-model leaderboard, and teardown REST-verified `{}` in all three Ray
regions with no Dataproc resource anywhere. The row above is earned.

It also took 218.9 minutes against the 122.6 of the pass it replaces, and the reason is worth more
than the row is.

**The number this page plans from moved.** The section above establishes 7.6 `neuralprophet`
cells/min per T4 as the planning figure, and `quota_and_scale.md` uses it. This run delivers **4.3**
— 10,000 cells across a 195.0-minute NeuralProphet cell span on twelve T4s. The two denominators are
not identical (7.6 was taken over the deep-learning job's 109.7-minute wall, which includes ramp that
the cell span excludes), so 4.3 is if anything the flattering reading. It is a 44 % regression
against a documented planning constant, and nothing in the config changed to cause it.

**The binding axis flipped from cores to the device.** The section above records the 2026-09-04
fleet running seven cells per T4 with 7 of 7 cores busy per node and the T4s at 0.7 of 1.0 — cores
decided the shape and the GPU fraction of `0.1` was slack. This run's executed plan
(`sizing_executed.deep_learning.gpu_pool`) is `derived_units 12`, `slots_per_unit 2`,
`total_slots 24`: two cells per card, with five of seven cores per node left idle. The GPU fraction
is now the smallest of the three terms the fleet sizer takes a minimum over, so it, not cores,
decides density.

**The fraction moved from `0.1` to `0.5` without any measurement behind either value.**
`compute.gpu_fraction` defaults to `"auto"`, which is meant to profile NeuralProphet's real device
footprint and size the per-task `num_gpus` to it. That profiling is executed in the driver, the
driver runs on the Ray head node, and the head has no accelerator — so the probe raises on every
sample and the function returns a fallback constant. It always has. Earlier code read the failed
probe as a measurement of zero and clamped to the *minimum* legal fraction, `0.1`; a correctness fix
stopped treating a failure as a measurement and falls back to a nominal `0.5` instead. The reasoning
improved and the number got worse. Neither `0.1` nor `0.5` was derived from anything measured.

**Density pays sublinearly, which is why the loss is 2x and not 3.5x.** Per-cell fit times across the
three densities this config has now been run at, all from `forecast_metadata`:

| Density | What bound it | Concurrent cells | Median fit | Cells/min |
|---|---|---|---|---|
| 1 per T4 | memory (the 2026-09-04 defect) | 12 | 29.0 s | 22.9 |
| 2 per T4 | **the GPU fraction (today)** | 24 | 26.0 s | 51.3 |
| 7 per T4 | cores | 84 | 42.6 s | 103.2 |

Each row is within about 10 % of `concurrency × 60 / median_fit`, so this is not a model fitted to
its answer. Going from seven cells a card to two cuts concurrency 3.5x but costs only 2.0x in
throughput, because seven fits contending for one T4 take 42.6 s each where two take 26.0 s. The
denser arm spends 64 % more time per cell to win 3.5x the parallelism, and still comes out ahead.

**What this does not establish.** It does not say what the right fraction is. The measured device
footprint — 50–78 KB on a 16 GB card, across 31,356 fits recorded elsewhere on this page — argues
the memory ceiling is nowhere near binding, but the table above shows contention is real even when
memory is not, so the right density is a throughput question this run did not sweep. It also does
...
for, because nobody set it up as one. It is the same 10,000 series and the same seven models as the
run above, on the same twelve-T4 fleet, submitted from the same code — and it differs in exactly one
thing that matters here: it **pins** `compute.gpu_fraction` at `0.125` instead of leaving it at
`"auto"`, so it never reaches the calibration at all. The launcher's preflight says so in a line
worth quoting, because it is the density decision made out loud: `this run would saturate at 1429
nodes (10000 cells / 7 per node)`. Seven cells a card, cores binding, the good arm.

| | GPU fraction | cells per T4 | concurrent cells | fits per cell | **fits/min per T4** |
|---|---|---|---|---|---|
...
every cell of the five Python models and none of the two BigQuery-native ones, exactly as intended.
COMPLETED in 303.2 minutes, and the persistent-resource listing came back `{}` in all three candidate
Ray regions afterwards with no Dataproc cluster or batch left behind. Over five hours the Ray Jobs
client refreshed its bearer token three times without the poll ever mistaking a token expiry for a
verdict on the run, which is the `ray_poll_recovery` axis doing the job it was added for.
...
The state this proves recoverable is the one that had no exit at all. Kill a launcher during the
twelve minutes a Vertex Ray cluster takes to build and you are left with two things that hold each
other in place: a cluster nobody owns, and a run header frozen at `RUNNING` whose job was never
submitted. Every repair verb refused it, each of them correctly — the probe could not reach a job
that did not exist, settle only writes verdicts it can defend, `close-runs` will not close a header
whose job row is non-terminal, and the reaper kept sparing the cluster *because* the header said
`RUNNING`. Six correct refusals and no way forward. It was cleared by hand the first time.

Three offline fixes landed earlier the same day. The one that matters here taught the Ray adapter
that a cluster we can reach, answering that it has no such job, is the strongest possible statement
that the job is absent — `NOT_FOUND`, which ages into `LOST` after the startup grace — rather than
`UNKNOWN`, which never ages into anything and is why the ladder stopped at its first rung.

**The fixture was killed on purpose, and that has to be said.** `kill -9` at six minutes into the
create is a faithful reproduction of the 2026-09-12 incident, where a dev machine restarted
mid-provision; it is not an independent sighting of the bug in the wild. What was manufactured is a
dead launcher, which is the thing under test. Nothing about the registry row or the cluster was
touched by hand at any point.

Five arms, each with its pass condition fixed before the launch, and all five passed as written:

| Arm | What had to happen | What happened |
|---|---|---|
| A — the incident is reproduced | orphaned cluster, `RUNNING` header, `RUNNING` job row carrying a `native_id`, zero cells | all four, at 20:29 UTC. The cluster was still `PROVISIONING` and went `RUNNING` on its own at 20:34, five minutes after its launcher died |
| B — the probe reaches a verdict | native `NOT_FOUND`, verdict `LOST` | `statistical ray RUNNING NOT_FOUND LOST 0/100 cluster alive; ray job not on it`. A verdict of `UNKNOWN` here would have been the pre-fix behaviour and a fail |
| C — settle writes what the probe earned | job row `FAILED` / `RUNTIME_LOST`, read back | preview said `RUNNING -> FAILED (runtime job gone; only 0/100 series landed)`; the forced write left exactly that, confirmed from `v_run_jobs` |
| D — `close-runs` closes what it could not | header terminal | `RUNNING → FAILED (every job FAILED)`, confirmed from `run_registry` |
| E — the reaper takes the cluster it was sparing | preview names it, `--yes` reclaims it, **verified by REST** | `run finished but the cluster is still up: deadlock-11h-d09a1bf19be9 is FAILED`; after `--yes` the endpoint returned `{}` and a direct GET returned 404 |

Arm D is listed separately even though it is purely downstream of arm C, because the claim being
tested is about the *set* of refusals rather than any one of them. Arm E's REST verification is not
ceremony either: the reaper's first live reclaim in the previous campaign printed `reaped 1 Ray
cluster(s)` over a cluster that was still `RUNNING`, so the SDK's own success line is precisely the
evidence that does not count here.

**One thing worth recording that is not a defect.** The settled job row's `ended_at` stayed null.
...
### The armed poll fault fired, and the evidence for it was invisible at the level the runbook logs at

`SF_RAY_POLL_FAULT` was built on 2026-09-10 so that the Ray poll recovery would stop depending on a
network blip arriving at a convenient moment. It was carried on a live run for the first time on
2026-09-22: `07_ray_cpu.json` re-submitted with `--force` under
`SF_RAY_POLL_FAULT=transport,auth`, which arms one 503 and one 401 into the opening polls of *each*
Ray job. Both families ran at attempt 2 under the unchanged `smoke-07-ray-cpu-ed27e03a2083`, the run
reached `COMPLETED` in 1,710.7 s, and the cluster was gone afterwards — the v1beta1 persistent-resource
endpoint returned `{}`, not the SDK's own success line.

**Four injections fired and not one retry line appeared.** The driver logged the arm four times —
twice for `statistical`, twice for `ml`, between 12:49:21 and 12:49:38 — and the log that follows
each one, the line saying the poll failed and the client is reconnecting, was nowhere. The runbook's
stated standard is that both lines appear and that one without the other is a finding, so by its own
rule this reads as a recovery that did not happen.

It is not. The cause is a log level. `_status_with_recovery` emitted its retry line at INFO while
the injection emitted at WARNING, and the harness — along with every invocation the runbook
describes — logs at WARNING. The one line that says the recovery fired could not be printed. A
reader following the runbook exactly would have read a working recovery as a broken one, which is a
worse failure than a missing feature: it turns the instrument into a source of false findings.

**The recovery did run, and the proof is stronger than the missing line would have been.** The run
completed, so `_status_with_recovery` never re-raised — under the old behaviour a single injected
503 would have terminated both jobs and torn the cluster down. And the two injections within each
job are about sixteen seconds apart, which is not a coincidence of timing: the only thing in that
...
classified it `TRANSIENT_CAPACITY`, backed off 120 seconds, and the second attempt succeeded. The
first attempt's resource was cleaned up rather than left behind. That path had been exercised
offline and by inference; this is it working unprompted on a run that was arming a different fault
entirely.

### An emitted launch command pasted verbatim creates the right job, and could not be pasted twice

The launcher prints a per-family `gcloud dataproc batches submit` line for every Spark family, and
the claim attached to it is that what is printed is what a launch would execute. On 2026-09-22 one
was pasted, with no edits, and it created a real batch:
`sf-03-serverless-gpu-paste-probe-f41401a9b314-deep-learning-a1`. The GPU overlay arrived intact and
was read back off the platform rather than off the command —
`spark.dataproc.executor.resource.accelerator.type = l4`, `executor.compute.tier = premium`,
`executor.disk.tier = premium`, and `SF_PROVISIONED_HARDWARE=gpu` in the executor environment.

The batch then failed before doing any work: `UNAVAILABLE / STOCKOUT` on L4 capacity in
`us-central1-a`. That is a capacity fact about that zone on that afternoon, not a defect, and it is
recorded because a campaign planning L4 work should know the pool is not always there.

**Being sloppy on the first try is part of the record.** The first attempt trimmed the `--properties`
string for readability and was rejected with *"When using an accelerator, you must configurate
compute tier to be 'premium'"*. The command was fine; the paste was not. The method that worked was
to have the planner write the exact emitted string to a file and execute the file, so nothing passed
through a human transcription step — which is what "pasted verbatim" has to mean if the claim is
going to be worth anything.

**The second finding is the one that changed code.** Pasting the emitted command for a config that
has *already* run is rejected with `ALREADY_EXISTS`, because the job key is the platform's own
`--batch` id and `dag.plan_dag` is pure: it stamps attempt 1 on everything, since "a planned identity
has no history to count". So the printed command for `smoke-03-serverless-gpu-92763e0f2242` named a
batch that had existed since 2026-09-09. Demonstrated directly on smoke 07, where `plan_run(force=True)`
emitted `…-statistical-a1` while `next_job_attempt(force=True)` returned attempt 3 — two attempts
stale.

**No offline test could have found this, and that is the interesting part.** Every test comparing an
emitted command to what the submitter builds does so from the same nodes, so both sides carried the
same stale `a1` and agreed. The only way to see it was to paste one at the real API. The fix puts the
history lookup in `launch_plan._nodes_with_submit_attempts` — where a registry is already being
consulted for the exists-vs-new verdict — rather than in `dag`, whose purity is load-bearing and
which must stay callable with no environment at all. `depends_on` is rewritten through the same map,
or the ensemble node would depend on family job ids that exist nowhere; if the registry is
