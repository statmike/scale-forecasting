...
| `mae`, `rmse`, `mse`, `mape`, `smape`, `wape`, `mase`, `rmsse`, `bias`, `coverage`, `pinball`, `mase_seasonal`, `maape`, `interval_score`, `interval_width` | `FLOAT64` | The metric panel — every metric, every run, so choosing a different `decision_metric` never means re-running. Populated only when a backtest produced out-of-fold predictions to score; otherwise NULL. Definitions in [configuration_reference.md](./configuration_reference.md#decision_metric). **These fifteen are what ships, not the schema's limit:** the column list is generated from the metric registry, so a deployment that adds a metric file gets its column here and the `ADD COLUMN IF NOT EXISTS` to back-fill it — see [adding_a_metric.md](./adding_a_metric.md). |
